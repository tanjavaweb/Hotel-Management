...
public class LoaiphongController {
	...
	@RequestMapping("/qllp")
	public String listlp(ModelMap model, @ModelAttribute("loaiphong") RoomType loaiphong) {
		activemenu(model);
		List<RoomType> llp = (List<RoomType>) iLoaiphongServices.findAll();// lấy list
		model.addAttribute("titlepage", "Room type list"); // tiêu đề cho trang
	...
	public String timdslp(ModelMap model, @ModelAttribute("loaiphong") RoomType loaiphong,
		...
		model.addAttribute("titlepage", "Room type list");
		return "qllp";
	}

	@RequestMapping("/addlp")
	public String addlp(ModelMap model, @ModelAttribute("loaiphong") RoomType loaiphong) {
	...
	public String actionaddlp(ModelMap model, @Validated @ModelAttribute("loaiphong") RoomType loaiphong,
	...
	public String edit(ModelMap model, @Validated @ModelAttribute("loaiphong") RoomType loaiphong,
			BindingResult errors) {
		activemenu(model);
		if (errors.hasErrors()) {
			model.addAttribute("titlepage", "Add new room type");
	...
	public String delete(ModelMap model, @ModelAttribute("loaiphong") RoomType loaiphong,
		...
		List<RoomType> l = (List<RoomType>) iLoaiphongServices.findAll();
	...
	public List<RoomType> phantranglp(int vitrihientai, List<RoomType> danhsach) {
		List<RoomType> l = danhsach;
		List<RoomType> lreturn = new ArrayList<>();
		// lay ra 10 item
	...
	public List<Integer> listSoLuongTrang(List<RoomType> danhsach, ModelMap model) {
	...
	public String lppage(ModelMap model, @RequestParam("page") int page,
			@ModelAttribute("loaiphong") RoomType loaiphong) {
		List<RoomType> l = (List<RoomType>) iLoaiphongServices.findAll();
		activemenu(model);
		vitrihientai = page;
	...
	public List<RoomType> phantranglptim(int vitrihientai, List<RoomType> danhsach) {
		List<RoomType> l = danhsach;
		List<RoomType> lreturn = new ArrayList<>();
		// lay ra 10 item
		for (int i = (vitrihientai - 1) * 10; i < (vitrihientai) * 10; i++) {
	...
	// số lượng button bấm chuyển trang
	public List<Integer> listSoLuongTrangtim(List<RoomType> danhsach, ModelMap model) {
		List<Integer> lreturn = new ArrayList<>();
		double temp = Double.parseDouble(danhsach.size() + "") / 10.0;
		int tempfor = (int) Math.ceil(temp);
		int a = 3;
