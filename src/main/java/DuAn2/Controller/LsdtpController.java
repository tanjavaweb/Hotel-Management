...
public class LsdtpController {
	...
	public String timlsdtp(ModelMap model, @ModelAttribute("datphong") Checkin DatPhong,
		...
		if (data2.isEmpty()) {
			PageRequest pageRequest = PageRequest.of(0, 10);
			ltim = (List<Checkin>) ilsdtp.findAllOrderByMaDesc(pageRequest);
			...
			try {
				...
				PageRequest pageRequest = PageRequest.of(0, 10);
				ltim = (List<Checkin>) ilsdtp.timlsdtp(data1, data2, datefind, pageRequest);
	...
	public String lsdtppagetim(ModelMap model, HttpServletRequest httpServletRequest, @RequestParam("page") int page) {
		...
		if (data2.isEmpty()) {
			...
			ltim = (List<Checkin>) ilsdtp.findAllOrderByMaDesc(pageable);
			count = ilsdtp.countFindAll();
		} else {
			try {
				datefind = dateFormat.parse(data2);
			...
			try {
				...
				ltim = (List<Checkin>) ilsdtp.timlsdtp(data1, data2, datefind, pageable);
				count = ilsdtp.countTimlsdtp(data1, data2, datefind);
			} catch (Exception e) {
				int data1 = 0;
				PageRequest pageable = PageRequest.of(page - 1, 10);
