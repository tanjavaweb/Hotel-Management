...
public class DsqldvController {
	...
	public String ViewListDsqldv(ModelMap model, @ModelAttribute("dichvu") Service dichvu) {
		activemenu(model);
		vitrihientai = 1;
		List<Service> l = (List<Service>) dsqldvService.findAllNotType3();
		model.addAttribute("titlepage", "Service list"); // tiêu đề cho trang
	...
	public String timdsqldv(ModelMap model, @ModelAttribute("dichvu") Service dichvu,
			HttpServletRequest httpServletRequest) {
		String data2 = httpServletRequest.getParameter("data");
		model.addAttribute("data", data2);
		List<Service> ltim;
		if (data2.isEmpty()) {
			ltim = (List<Service>) dsqldvService.findAllNotType3();
		} else {

			try {
				int data1 = Integer.parseInt(data2);
				ltim = (List<Service>) dsqldvService.timldsdv(data1, data2);
			} catch (Exception e) {
				int data1 = 0;
				ltim = (List<Service>) dsqldvService.timldsdv(data1, data2);
			}
	...
	public String deleteqldv(ModelMap model, @ModelAttribute("dichvu") Service dichvu, @RequestParam("id") int id) {
		activemenu(model);
		model.addAttribute("titlepage", "Service list");
	...
	public String actionsuadsqldv(ModelMap model, @Validated @ModelAttribute("dichvu") Service dichvu,
	...
	public String themdsqldv(ModelMap model, @ModelAttribute("dichvu") Service dichvu) {
	...
	public String actionthemdsqldv(ModelMap model, @Validated @ModelAttribute("dichvu") Service dichvu,
			BindingResult errors) {
		// cai dat. tat ca duoi deu show tren cai dat --------------------
		model.addAttribute("chamshowcd", ".show");
	...
	public List<Service> phantrangdsdv(int vitrihientai, List<Service> danhsach) {
		List<Service> l = danhsach;
		List<Service> lreturn = new ArrayList<>();
//lay ra 10 item
	...
	public List<Integer> listSoLuongTrang(List<Service> danhsach, ModelMap model) {
...
//-------------
	...
	public String dsqldvpage(ModelMap model, @RequestParam("page") int page, @ModelAttribute("dichvu") Service dichvu) {
		activemenu(model);
		List<Service> l = (List<Service>) dsqldvService.findAllNotType3();
		model.addAttribute("titlepage", "Service list");
		vitrihientai = page;
	...
	public List<Service> phantrangdsdvtim(int vitrihientai, List<Service> danhsach) {
		List<Service> l = danhsach;
		List<Service> lreturn = new ArrayList<>();
		// lay ra 10 item
		for (int i = (vitrihientai - 1) * 10; i < (vitrihientai) * 10; i++) {
	...
	// số lượng button bấm chuyển trang
	public List<Integer> listSoLuongTrangtim(List<Service> danhsach, ModelMap model) {
		List<Integer> lreturn = new ArrayList<>();
		double temp = Double.parseDouble(danhsach.size() + "") / 10.0;
		int tempfor = (int) Math.ceil(temp);
		int a = 3;
	...
	public String dsqldvpagetim(ModelMap model, @RequestParam("page") int page, @ModelAttribute("dichvu") Service dichvu,
			HttpServletRequest httpServletRequest) {
		activemenu(model);
		model.addAttribute("titlepage", "Service list");
		vitrihientai = page;
