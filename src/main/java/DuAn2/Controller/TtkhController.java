...
public class TtkhController {
	...
	public String ttkh(ModelMap model) {
		...
		List<Checkin> l = (List<Checkin>) ittkhService.findAllOrderBy(pageable);
		vitrihientai = 1;
		model.addAttribute("titlepage", "Customer information");
	...
	public String timttkh(ModelMap model, @RequestParam("data") String data) {
		activemenu(model);
		PageRequest pageable = PageRequest.of(0, 10);
		List<Checkin> ltim = ittkhService.timttkh(data, pageable);
